...
El prier argumento es la nombre del objeto que contiene la tabla que queremos procesar (`mtcars`), el segundo argumento, `select` es una cadena de caracteres que contenga los nombres de las columnas que queremos extraer (resultado en tabla \@ref(tab:mtcars.1)).
...
donde hemos creado un nuevo objeto `mtcars.desc` ordenado de mayor a menor (descendiente) con base en la columna `mpg`. Para ordenar ascendentemente (tabla \@ref(tab:mtcars.desc)):

```{r echo = T}
mtcars.asc <- mtcars %>% arrange(mpg)
```

```{r mtcars.desc, echo = F}
